...
-------------------------------------------------------------------------------------------
DECLARE @CategoriaMasAmplia AS VARCHAR(12) = (SELECT TOP(1) CP.nombre
																						  FROM CATEGORIA_PRODUCTOS CP JOIN PRODUCTOS P
																							ON cp.codCategoria = P.codCategoria
																						 GROUP BY CP.nombre
...
DECLARE @codEmpleado7 INT = 7,
				@Empleado7 VARCHAR(40),
				@JefeEmpleado7 VARCHAR(40)

SELECT @Empleado7 = CONCAT(E.nombre, ' ', E.apellido1, ' ', E.apellido2), @JefeEmpleado7 = (CONCAT(EJ.nombre, ' ', EJ.apellido1, ' ', Ej.apellido2))
...
SELECT @numPedidos = COUNT(DP.codPedido)-- , @nombreCliente9 = C.nombre_cliente
  FROM PEDIDOS P LEFT JOIN DETALLE_PEDIDOS DP
	ON P.codPedido = DP.codPedido LEFT JOIN CLIENTES C
	ON c.codCliente = p.codCliente
WHERE P.codCliente = @codCliente9

PRINT CONCAT('El cliente ', @codCliente9 )
